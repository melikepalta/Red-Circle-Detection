...
The script reads every image in the `test_images` folder and runs each one through a detection pipeline with four filters:
...
2. **Contour Area** — Ignores shapes that are too small to be meaningful (default minimum: 300px²).
...
```
.
├── test_images/        # Input images (place your 100 images here)
...
1. Put your images in the `test_images` folder.
2. Run the script:
